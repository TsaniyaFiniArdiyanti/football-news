...
def login_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response.set_cookie('last_login', str(datetime.datetime.now())) # untuk mendaftarkan cookie last_login di response dengan isi timestamp terkini.
            return response   
            # user = form.get_user()
            # login(request, user)
            # return redirect('main:show_main')
...
def logout_user(request):
    ...
    response.delete_cookie('last_login') # menghapus cookie
    return response
